...
const Input = (props: InputProps) => {
    ...
    const handleChange = useCallback((payload: unknown, ...otherArgs: unknown[]) => {
        ...
        } else if (payload && typeof payload === 'object' && 'nativeEvent' in payload && payload.nativeEvent instanceof Event) {
    ...
    return (
        <div ref={inputRef} className={containerClassName} style={{outline: 'none'}} tabIndex={-1}>
            ...
            />
            {hasError && <span className="text-red-500 text-xs mt-1 block">{fieldProps.errorMessage}</span>}
        </div>
    );
};
...
const Form = React.forwardRef<FormRef, FormProps>((props, ref) => {
    ...
    const handleSubmitForm = useCallback((evnt: React.FormEvent<HTMLFormElement>) => {
        evnt.preventDefault();
         let hasError = false;
        for (const key of Object.keys(fields)) {
             const value = formData.get(key);
            if(fields[key] && fields[key].required && (!value || ['undefined', 'null'].includes(String(value)))) {
    ...
    const handleFormChange = useCallback((payload: unknown) => {
